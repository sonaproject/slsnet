...
public class SlsNetL2NetworkNeighbour {
    ...
    protected void handleRequest(NeighbourMessageContext context) {
        ...
        if (l2Network != null) {
            log.debug("slsnet handle neightbour request message: {}", context.inPort());
            l2Network.interfaces().stream()
                    .filter(intf -> !context.inPort().equals(intf.connectPoint()))
                    .forEach(context::forward);
        } else {
            log.warn("slsnet handle neightbour request message: {} --> DROP UNKNOWN", context.inPort());
            context.drop();
        }
    }
