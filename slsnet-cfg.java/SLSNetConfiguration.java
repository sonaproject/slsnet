...
@Service
public class SLSNetConfiguration implements
        SLSNetConfigurationService {
    ...
    public void activate() {
        ...
        registry.registerConfigFactory(SLSNetConfigFactory);
        setUpConfiguration();
        log.info("Simple Leaf-Spine Network configuration service started");
    }

    @Deactivate
    public void deactivate() {
        registry.unregisterConfigFactory(SLSNetCOnfigFactory);
        configService.removeListener(configListener);
        log.info("Simple Leaf-SPine Network configuration service stopped");
    }
    ...
    private void setUpConfiguration() {
        SLSNetConfig config = configService.getConfig(
                coreService.registerApplication(SLSNetConfigurationService.SLSNET_APP_ID),
                SLSNetConfigurationService.CONFIG_CLASS);
        if (config == null) {
            log.warn("No SLSNET config available!");
            return;
        }
    ...
    private class InternalNetworkConfigListener implements NetworkConfigListener {
        ...
        public void event(NetworkConfigEvent event) {
            ...
            case CONFIG_UPDATED:
            case CONFIG_REMOVED:
                if (event.configClass() == SLSConfigurationService.CONFIG_CLASS) {
                    setUpConfiguration();
                }
                break;
            default:
